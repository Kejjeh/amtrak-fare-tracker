...
`amtrak-nhv-bos-weekend-fare-tracker` (daily ~6 AM). It appends rows to the fare
log. To publish to GitHub Pages, add the **push step** at the bottom.
...
## PUSH STEP (add this when publishing to GitHub Pages; requires GitHub connector authorized)
After appending rows, commit and push the updated CSV to the Pages repo:
`git add data/amtrak_fare_log.csv && git commit -m "fares: {today}" && git push`
